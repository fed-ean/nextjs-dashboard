...
export default function TarjetaNoticia({ post }) {
  ...
  const imageUrl = post.featuredImage?.node?.sourceUrl || extractImageUrl(post.content);

  // Se limpia el 'excerpt' de HTML y se recorta para una vista previa.
  const cleanExcerpt = stripHtml(post.excerpt || '').substring(0, 120) + (post.excerpt?.length > 120 ? '...' : '');
