...
const RadioPlayerUI: React.FC<RadioPlayerUIProps> = ({
  playing,
  volume,
  onPlayPause,
  onVolumeChange
        ...
        <div className="flex flex-col items-center space-y-4 w-full max-w-xl">
          ...
          <div className="flex items-end h-24 w-full space-x-1">
            {Array.from({ length: totalBars }).map((_, barIndex) => {
              const waveValue =
                Math.sin((barIndex / totalBars) * Math.PI + animationTrigger) *
                (0.5 + Math.random() * 0.5);
              ...
              return (
                <div
                  key={barIndex}
                  className="flex flex-col-reverse justify-start items-center space-y-1 h-full"
                >
                  {Array.from({ length: totalBlocks }).map((_, blockIndex) => {
                    const blockIsActive = blockIndex < barHeight;
                    const blockColor =
                      barIndex < totalBars / 2 ? 'bg-red-600' : 'bg-purple-600';

                    return (
                      <div
                        key={blockIndex}
                        className={`w-3 h-3 ${
                          blockIsActive ? blockColor : 'bg-transparent'
                        } rounded-sm`}
                      ></div>
                    );
                  })}
